...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'user_social_stats_flame_stages_valid'
  ) then
    alter table public.user_social_stats
  ...
  if not exists (
    select 1
    from pg_constraint
    where conname = 'user_social_stats_flame_quests_valid'
  ) then
    alter table public.user_social_stats
  ...
  if not exists (
    select 1
    from pg_constraint
    where conname = 'user_social_stats_selected_flame_effect_valid'
  ) then
    alter table public.user_social_stats
      add constraint user_social_stats_selected_flame_effect_valid
      check (
        selected_flame_effect is null
        or selected_flame_effect in (
          'seven-lights',
          'prismatic-halo',
          'comet-trail',
          'constellation',
...
returns table (
  ...
  week_revision_daily_average numeric,
  total_stars integer,
  best_streak integer,
  best_run integer,
  flame_stages text[],
  flame_quests text[],
  selected_flame_effect text
)
language plpgsql
security definer
...
begin
  ...
  select
    ...
    profile.friend_code,
    coalesce(stats.week_stars, 0),
    coalesce(stats.current_streak, 0),
    coalesce(stats.week_revisions_done, 0),
    coalesce(stats.week_revision_daily_average, 0),
    coalesce(stats.total_stars, 0),
    coalesce(stats.best_streak, 0),
    coalesce(stats.best_run, 0),
    coalesce(stats.flame_stages, '{}'::text[]),
    coalesce(stats.flame_quests, '{}'::text[]),
    stats.selected_flame_effect
  from (select 1) anchor
  left join public.user_social_stats stats
    on stats.user_id = profile.id
    and stats.week_start = week_anchor;
end;
$$;
...
as $$
  ...
  select
    ...
    coalesce(stats.week_revision_daily_average, 0) as week_revision_daily_average,
    coalesce(stats.total_stars, 0) as total_stars,
    coalesce(stats.best_streak, 0) as best_streak,
    coalesce(stats.best_run, 0) as best_run,
    coalesce(stats.flame_stages, '{}'::text[]) as flame_stages,
    coalesce(stats.flame_quests, '{}'::text[]) as flame_quests,
    stats.selected_flame_effect
  from friend_ids
...
as $$
  ...
  left join public.user_social_stats stats
    ...
    and stats.week_start = week_anchor.week_start
  order by
    coalesce(stats.total_stars, 0) desc,
    coalesce(stats.week_stars, 0) desc,
    coalesce(stats.current_streak, 0) desc,
    coalesce(stats.week_revision_daily_average, 0) desc,
